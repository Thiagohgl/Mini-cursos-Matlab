...
% são as features
 ...
 X_treino = cancerInputs(1:200,:);
 Y_treino = cancerTargets(1:200);
 
 X_teste = cancerInputs(200:end,:);
 Y_teste = cancerTargets(200:end);

 Mdl = fitcknn(X_treino,Y_treino,'NumNeighbors',5);
 
 Error_treino = resubLoss(Mdl);

 Y_predict = Mdl.predict(X_teste);
...
% decision tree para conseguir o resultado. 
 ...
 Mdl = fitctree(X_treino,Y_treino);
...
%% Exercício 3
% Tente prever se um aluno precisa de ajuda ou não baseado
...
[net,tr] = train(net,X_treino',Y_treino');

% Test the Network
outputs = net(X_teste');
errors = gsubtract(Y_teste',outputs);
performance = perform(net,Y_teste',outputs)


 %Mdl = fitctree(X_treino,Y_treino);
  Mdl = fitcknn(X_treino,Y_treino,'NumNeighbors',3);
