...
% Crie as opções para a função de otimização

% Gradiente numérico
%options = optimoptions('fminunc','Display','iter','Algorithm','quasi-newton','MaxFunEvals',1000); % Sem gradiente
% Gradiente dado na função
options = optimoptions('fminunc','Algorithm','trust-region','Display','iter','MaxFunEvals',1000);
...
problem.x0 = [20,10,5];
problem.lb = [5 ,5,5]; % Limite mínimo em cada ponto
problem.ub = [100, 100,100]; % Limite máximo em cada ponto
problem.Aeq = [1,2,4];
problem.Beq = 115;
...
%% Parte 4: Nelder-Mead
% Nelder-Mead é um algoritmo de otimização 
...
V = pi*X.^2.*Y;
V = V >= 650; % Volumes menors que 650 são descartados
A = 2*pi.*X.*Y;
A = A.*V;
figure
surf(X,Y,A)
